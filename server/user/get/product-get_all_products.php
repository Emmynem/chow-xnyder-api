  ...
  if ($connected) {
    ...
    try {
      ...
      if ($functions->validateInt($start_limit) == false && $start_limit != "0") {
        ...
        LEFT JOIN menus ON products.menu_unique_id = menus.unique_id WHERE products.status=:status ORDER BY products.added_date ASC, products.views DESC, products.favorites DESC, products.good_rating DESC LIMIT ".$start_limit.",".$end_limit."";
