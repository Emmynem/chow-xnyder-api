<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $sql = "SELECT carts.unique_id, carts.vendor_unique_id, vendors.business_name as vendor_name, vendors.stripped as vendor_stripped, carts.product_unique_id, carts.quantity, carts.shipping_fee_unique_id, carts.pickup_location, carts.added_date,
      products.name, products.stripped, products.duration, products.weight, products.stock, products.stock_remaining, products.price, products.sales_price, products.views, shipping_fees.price as shipping_price FROM carts
      INNER JOIN vendors ON carts.vendor_unique_id = vendors.unique_id LEFT JOIN products ON carts.product_unique_id = products.unique_id LEFT JOIN shipping_fees ON carts.shipping_fee_unique_id = shipping_fees.unique_id WHERE carts.user_unique_id=:user_unique_id AND carts.status=:status ORDER BY carts.added_date DESC";
