<?php
  ...
  $limit = isset($_GET['limit']) ? $_GET['limit'] : $data["limit"];
  ...
  if ($connected) {
    ...
    try {
      ...
      LEFT JOIN products ON favorites.product_unique_id = products.unique_id LEFT JOIN vendors ON products.vendor_unique_id = vendors.unique_id WHERE favorites.user_unique_id=:user_unique_id AND favorites.status=:status ORDER BY favorites.added_date DESC LIMIT ".$limit;
