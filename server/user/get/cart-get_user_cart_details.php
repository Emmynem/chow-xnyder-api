<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $sql = "SELECT carts.unique_id, carts.vendor_unique_id, vendors.business_name as vendor_name, vendors.stripped as vendor_stripped, carts.product_unique_id, carts.quantity, carts.shipping_fee_unique_id, carts.pickup_location, carts.added_date, carts.last_modified, carts.status,
      products.name, products.stripped, products.duration, products.weight, products.stock, products.stock_remaining, products.price, products.sales_price, products.views, products.favorites, products.good_rating, products.bad_rating, shipping_fees.price as shipping_price FROM carts
      INNER JOIN vendors ON carts.vendor_unique_id = vendors.unique_id LEFT JOIN products ON carts.product_unique_id = products.unique_id LEFT JOIN shipping_fees ON carts.shipping_fee_unique_id = shipping_fees.unique_id WHERE carts.unique_id=:unique_id AND carts.user_unique_id=:user_unique_id
      AND carts.status=:status ORDER BY carts.added_date DESC";
      ...
      if ($query->rowCount() > 0) {
        foreach ($result as $key => $value) {
          ...
          $current_cart['vendor_unique_id'] = $value['vendor_unique_id'];
