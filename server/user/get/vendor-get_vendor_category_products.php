  ...
  if ($connected) {
    ...
    try {
      ...
      if ($functions->validateInt($start_limit) == false && $start_limit != "0") {
        ...
        $sql2 = "SELECT unique_id, stripped FROM vendors WHERE stripped=:stripped AND status=:status";
        $query2 = $conn->prepare($sql2);
        $query2->bindParam(":stripped", $vendor_stripped);
        ...
        if ($query2->rowCount() > 0) {
          ...
          $vendor_unique_id = $result2[0];

          $sql = "SELECT unique_id, stripped FROM categories WHERE stripped=:stripped AND status=:status";
          ...
          if ($query->rowCount() > 0) {
            ...
            LEFT JOIN menus ON products.menu_unique_id = menus.unique_id WHERE products.vendor_unique_id=:vendor_unique_id AND products.category_unique_id=:category_unique_id AND products.status=:status ORDER BY products.added_date ASC, products.views DESC, products.favorites DESC, products.good_rating DESC LIMIT ".$start_limit.",".$end_limit."";
