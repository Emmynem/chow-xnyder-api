          ...
          if ($queryTracker->rowCount() > 0) {
            ...
            if (!is_array($order_unique_ids)) {
              ...
              foreach ($order_unique_ids as $key => $the_order_unique_id){
                ...
                $sqlOrder = "SELECT delivery_status, vendor_unique_id, service_charge FROM orders WHERE unique_id=:unique_id AND user_unique_id=:user_unique_id AND status=:status";
                ...
                if ($queryOrder->rowCount() > 0) {
                  ...
                  $vendor_unique_id = $the_order_details[1];
