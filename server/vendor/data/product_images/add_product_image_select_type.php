    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        if ($query2->rowCount() > 0) {
          ...
          $sql3 = "SELECT unique_id FROM product_images WHERE product_unique_id=:product_unique_id AND vendor_unique_id=:vendor_unique_id AND status=:status";
          ...
          if (empty($_FILES['file']['name'])) {
            ...
            $returnvalue = new genericClass();
            $returnvalue->engineError = 2;
            $returnvalue->engineErrorMessage = "Item images maxed out";
          }
                              ...
                              if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                ...
                                $targetPath = $path; // Target path where file is to be stored
                                 ...
                                 $file = $path;
                                  ...
                                  $sql = "INSERT INTO product_images (unique_id, user_unique_id, vendor_unique_id, product_unique_id, image, file, file_size, added_date, last_modified, status)
                                  VALUES (:unique_id, :user_unique_id, :vendor_unique_id, :product_unique_id, :image, :file, :file_size, :added_date, :last_modified, :status)";
