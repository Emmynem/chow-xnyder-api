  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $validation = $functions->update_vendor_product_validation($category_unique_id, $name, $description, $duration, $weight, $price, $sales_price);
        ...
        if ($validation["error"] == true) {
          ...
          if ($query2->rowCount() > 0) {
            ...
            $sql3 = "SELECT name FROM products WHERE vendor_unique_id=:vendor_unique_id AND (menu_unique_id=:menu_unique_id OR menu_unique_id IS NULL)
            AND category_unique_id=:category_unique_id AND (name=:name OR stripped=:stripped) AND unique_id!=:unique_id AND status=:status";
            $query3 = $conn->prepare($sql3);
            $query3->bindParam(":unique_id", $unique_id);
            $query3->bindParam(":vendor_unique_id", $vendor_unique_id);
            $query3->bindParam(":menu_unique_id", $menu_unique_id);
            ...
            if ($query3->rowCount() > 0) {
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Item name already exists";
            }
            else {
              ...
              $sql = "UPDATE products SET menu_unique_id=:menu_unique_id, category_unique_id=:category_unique_id,
              name=:name, stripped=:stripped, description=:description, duration=:duration, weight=:weight, price=:price, sales_price=:sales_price, last_modified=:last_modified WHERE unique_id=:unique_id AND vendor_unique_id=:vendor_unique_id";
              ...
              if ($query->rowCount() > 0) {
                ...
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Not edited (item details)";
              }
