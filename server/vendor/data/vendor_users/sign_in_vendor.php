<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $sql2 = "SELECT access, unique_id, role FROM vendor_users WHERE email=:email OR phone_number=:phone_number AND status=:status";
      ...
      if ($query2->rowCount() > 0) {
        ...
        $user_role = $the_access_details[2];

        switch ($access) {
          case $functions->granted:
            $sql = "UPDATE vendor_users SET fullname=:fullname, email=:email, phone_number=:phone_number, image=:image, last_modified=:last_modified WHERE unique_id=:unique_id";
