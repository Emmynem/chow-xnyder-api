        ...
        if (in_array($user_unique_id,$authorized_users)) {
          ...
          if ($query4->rowCount() > 0) {
            ...
            if ($query2->rowCount() > 0) {
              ...
              $new_service_charge = $service_charge - $amount;

              $sql3 = "UPDATE vendors SET service_charge=:service_charge, last_modified=:last_modified WHERE unique_id=:unique_id";
              $query3 = $conn->prepare($sql3);
              $query3->bindParam(":unique_id", $vendor_unique_id);
              $query3->bindParam(":service_charge", $new_service_charge);
              ...
              if ($query3->rowCount() > 0) {
                ...
                if ($query->rowCount() > 0) {
                  ...
                  $returnvalue->engineErrorMessage = "Not completed (service charge payment)";
                }
                ...
                $returnvalue = new genericClass();
                $returnvalue->engineError = 2;
                $returnvalue->engineErrorMessage = "Not updated (vendor's service charge)";
              }
