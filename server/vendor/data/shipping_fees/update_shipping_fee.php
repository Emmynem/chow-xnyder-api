  ...
  if ($connected) {
    ...
    try {
      ...
      if ($querySearchUser->rowCount() > 0) {
        ...
        $validation = $functions->vendor_shipping_fees_validation($city, $state, $country, $price);
        ...
        if ($validation["error"] == true) {
          ...
          $sql2 = "SELECT unique_id FROM shipping_fees WHERE unique_id=:unique_id AND product_unique_id=:product_unique_id AND vendor_unique_id=:vendor_unique_id AND status=:status";
          ...
          if ($query2->rowCount() > 0) {
            ...
            $sql3 = "SELECT city FROM shipping_fees WHERE city=:city AND state=:state AND country=:country AND product_unique_id=:product_unique_id AND vendor_unique_id=:vendor_unique_id AND unique_id!=:unique_id AND status=:status";
            ...
            if ($query3->rowCount() > 0) {
              $returnvalue = new genericClass();
              $returnvalue->engineError = 2;
              $returnvalue->engineErrorMessage = "Item location already exists";
            }
            else {
              $sql = "UPDATE shipping_fees SET city=:city, state=:state, country=:country, price=:price, last_modified=:last_modified WHERE unique_id=:unique_id AND product_unique_id=:product_unique_id AND vendor_unique_id=:vendor_unique_id";
