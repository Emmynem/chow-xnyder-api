<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      // $data['category_image_unique_id'] = $_POST['category_image_unique_id'];
      // $data['user_unique_id'] = $_POST['user_unique_id'];
      // $data['category_unique_id'] = $_POST['category_unique_id'];
      // $data['upload_type'] = $_POST['upload_type'];

      $category_image_unique_id = isset($_POST['category_image_unique_id']) ? $_POST['category_image_unique_id'] : $data['category_image_unique_id'];
      ...
      // if ($querySearchUser->rowCount() > 0) {
        ...
        if (in_array($user_unique_id,$authorized_users)) {
          ...
          $sql3 = "SELECT file FROM category_images WHERE unique_id=:unique_id AND category_unique_id=:category_unique_id AND status=:status";
          $query3 = $conn->prepare($sql3);
          $query3->bindParam(":unique_id", $category_image_unique_id);
          ...
          if (empty($_FILES['file']['name'])) {
            ...
            $category_image_details = $query3->fetch();
            $old_category_image = $category_image_details[0];
                              ...
                              if((isset($newFilename)) && ($newFilename != "Image already exist")){
                                ...
                                if(compressImage($sourcePath, $path, 60) == 1){
                                 $file = $path;
                                  ...
                                  $sql = "UPDATE category_images SET image=:image, file=:file, file_size=:file_size, last_modified=:last_modified
                                  WHERE unique_id=:unique_id AND category_unique_id=:category_unique_id";
                                  $query = $conn->prepare($sql);
                                  $query->bindParam(":unique_id", $category_image_unique_id);
