<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      users.email as user_email, users.phone_number as user_phone_number, users.phone_number as user_phone_number, products.name, products.stripped, products.duration, products.weight, products.stock, products.stock_remaining, products.price, products.sales_price, products.views, products.favorites, products.good_rating, products.bad_rating, shipping_fees.price as shipping_price FROM carts
      INNER JOIN users ON carts.user_unique_id = users.unique_id INNER JOIN vendors ON carts.vendor_unique_id = vendors.unique_id LEFT JOIN products ON carts.product_unique_id = products.unique_id LEFT JOIN shipping_fees ON carts.shipping_fee_unique_id = shipping_fees.unique_id
      WHERE (carts.added_date=:start_date OR carts.added_date >:start_date) AND (carts.added_date <:end_date OR carts.added_date=:end_date) ORDER BY carts.added_date DESC";
