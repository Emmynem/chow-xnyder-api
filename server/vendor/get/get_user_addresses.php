<?php
  ...
  if ($connected) {
    ...
    try {
      ...
      $sql2 = "SELECT show_addresses FROM users_preferences WHERE user_unique_id=:user_unique_id";
      $query2 = $conn->prepare($sql2);
      $query2->bindParam(":user_unique_id", $user_unique_id);
      $query2->execute();

      $the_user_preference_details = $query2->rowCount() > 0 ? $query2->fetch() : null;
      $show_addresses = $the_user_preference_details != null ? (int)$the_user_preference_details[0] : null;
      ...
      if ($show_addresses == 1) {
        ...
        $sql = "SELECT users_addresses.user_unique_id, users_addresses.firstname, users_addresses.lastname, users_addresses.address,
        users_addresses.additional_information, users_addresses.city, users_addresses.state, users_addresses.country, users_addresses.default_status, users_addresses.added_date, users_addresses.last_modified, users_addresses.status, users.fullname as user_fullname FROM users_addresses
